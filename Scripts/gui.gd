...
func _process(_delta: float) -> void:
	...
	if Input.is_action_just_pressed("pausar") and players > 0:
		# A árvore já pausa a ação com Engine.time_scale. Não abre dois menus juntos.
		if tela_upgrades.has_method("esta_aberta") and bool(tela_upgrades.call("esta_aberta")):
			return
