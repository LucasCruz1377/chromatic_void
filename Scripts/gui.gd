...
func _ajustar_hud_responsivo() -> void:
	# CanvasLayer não acompanha a Camera2D. Sua origem é o canto do viewport,
	# não a origem negativa da arena expandida. Todas as peças usam este retângulo.
...
func _aplicar_layout_hud(tamanho: Vector2) -> void:
	...
	for barra in [barra_vida, barra_xp]:
		if not is_instance_valid(barra):
			continue
		# Sem nine_patch, a textura de 630 px impede o Control de encolher.
