...
func _ready() -> void:
	...
	await get_tree().process_frame
	verificar(
		get_tree().get_nodes_in_group("projetil_primaveril").size() == 10,
		"a onda da fase 3 não lançou dez espinhos"
	)
	limpar_projeteis()
	await get_tree().process_frame
	...
	verificar(boss.petalas_em_voo == 3, "a fase 3 não lançou três pétalas")
	verificar(contar_petalas_ocultas() == 3, "as pétalas lançadas continuaram presas ao corpo")
	verificar(
		get_tree().get_nodes_in_group("projetil_primaveril").size() == 3,
